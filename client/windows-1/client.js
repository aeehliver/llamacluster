...
class WindowsClient extends EventEmitter {
    constructor() {
        super();
        this.id = `windows-1-${Math.random().toString(36).substr(2, 9)}`;
    ...
    connectToServer() {
        ...
        try {
            ...
            console.log(`[WS] Conectando al servidor en: ${wsUrl}`);
            ...
            this.ws.on('close', () => {
                ...
                if (this.reconnectAttempts < this.MAX_RECONNECT_ATTEMPTS) {
                    ...
                    console.log(`[WS] Intento de reconexión ${this.reconnectAttempts}/${this.MAX_RECONNECT_ATTEMPTS} en ${this.RECONNECT_DELAY}ms...`);
    ...
    handleModelPartition(partition) {
        ...
        // Simular carga del modelo más rápido
        const loadInterval = setInterval(() => {
            this.modelStatus.progress += 25; // Incrementar en 25% cada vez
            ...
            if (this.modelStatus.progress >= 100) {
                ...
                // Enviar actualización de estado inmediatamente
                this.sendStatusUpdate();
            }
            
            // Enviar actualización de estado en cada progreso
            this.sendStatusUpdate();
        }, 500); // Reducido a 500ms entre actualizaciones
    }
