...
class P2PDiscovery extends EventEmitter {
    ...
    stop() {
        ...
        try {
            ...
            // Cerrar el socket solo si existe y está en ejecución
            if (this.socket) {
                const tempSocket = this.socket;
                this.socket = null; // Limpiar la referencia primero
                
                try {
                    tempSocket.close(() => {
                        console.log('Socket UDP cerrado correctamente');
                    });
                } catch (error) {
                    console.error('Error al cerrar el socket:', error);
                }
            }
            ...
            Object.values(interfaces).forEach(iface => {
                iface.forEach(addr => {
                    if (addr.family === 'IPv4' && !addr.internal) {
                        ...
                        console.log(`Enviando broadcast a ${broadcastAddr}`);
                        this.socket.send(buffer, 0, buffer.length, this.port, broadcastAddr);
                        // También enviar a la dirección de red
                        const networkAddr = this.getNetworkAddress(addr.address, addr.netmask);
                        console.log(`Enviando a la red ${networkAddr}`);
                        this.socket.send(buffer, 0, buffer.length, this.port, networkAddr);
                    }
                });
            });

            // También enviar a broadcast general
            console.log('Enviando broadcast general');
    ...
    startCleanup() {
        this.cleanupInterval = setInterval(() => {
            ...
            for (const [nodeId, peer] of this.peers.entries()) {
                if (now - peer.lastSeen > 15000) { // 15 segundos de timeout
    ...
    handleMessage(message, rinfo) {
        if (message.nodeId === this.nodeId) return; // Ignorar mensajes propios
        ...
        if (isNewPeer) {
            console.log(`Nuevo peer descubierto: ${peer.hostname} (${peer.nodeId}) en ${peer.address}`);
            this.emit('peer-discovered', { nodeId: message.nodeId, hostname: message.hostname });
            this.updateLeader();
        }

        if (message.type === 'broadcast') {
            console.log(`Mensaje broadcast recibido de ${peer.hostname} (${peer.nodeId}):`, message.data);
